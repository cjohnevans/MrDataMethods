function [spectro] = cubspec_fida_proc( spectro_file_list , ...
    water_file_list, ...
    phase0, ...
    phase1, ...
    phase_ppm, ...
...
%    water_file_list:   cell array of water ref data = {''} for no water data
%    phase0:            zeroth order phase 
%    phase1:            first order phase
%    phase_ppm:         centre pivot point for phase correction
...
for file = 1:n_files
    ...
    spectro{file} = op_addphase(spectro{file}, phase0, phase1, phase_ppm,0); % phase relative to naa, thal
